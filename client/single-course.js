...
function updateCoursePage(course, isEnrolled) {
    ...
    // 2. ברירת מחדל – לפני בחירת שיעור
    const videoPlayer = document.getElementById('videoPlayer');
    videoPlayer.innerHTML = `
      <div class="video-placeholder">
        <p>בחרי שיעור מתוך הרשימה כדי להתחיל לצפות 🎬</p>
      </div>
    `;
    document.getElementById('youtubeTitle').textContent = 'בחרי שיעור לצפייה';
    document.getElementById('youtubeDescription').textContent = '';

    // 3. יצירת הפלייליסט הדינמי
    const playlistList = document.getElementById('playlistList');
    playlistList.innerHTML = ''; 
    ...
    if (course.courseContent && course.courseContent.length > 0) {
        ...
        // פונקציה גלובלית שמופעלת בלחיצה על שיעור בפלייליסט
       window.changeVideo = (videoId, title, description) => {
    console.log("🎬 changeVideo called with:", videoId, title);

    if (!videoId || videoId === 'null' || videoId === 'undefined') {
        alert('לשיעור זה לא הוגדר סרטון YouTube.');
        return;
    }

    // כותרת/תיאור מהקורס – רואים מיד
    document.getElementById('youtubeTitle').textContent = title;
    document.getElementById('youtubeDescription').textContent = description || '';

    // ואז – קריאה לפונקציה שמדברת עם YouTube API + מטמיעה וידאו
    if (typeof fetchYouTubeVideo === 'function') {
        fetchYouTubeVideo(videoId);
    }
    ...
    document.querySelector(`[data-video-id='${videoId}']`)?.classList.add('active');
        ...
        playlistList.innerHTML = '<p style="text-align: center; color: #555;">לא נמצאו שיעורים בקורס זה.</p>';
        videoPlayer.innerHTML = '<p style="text-align:center;">אין תוכן זמין.</p>';
    }
}


// פונקציית עזר להגנה על HTML (מונעת תקלות כשהכותרת כוללת גרשיים/תגים)
...
function createLessonElement(lesson, isEnrolled) {
    ...
    if (isPlayable) {
        ...
        div.setAttribute(
            'onclick',
            `changeVideo('${lesson.videoId}', '${cleanTitle.replace(/'/g, "\\'")}', '${cleanDesc.replace(/'/g, "\\'")}')`
        );
    } else {
        div.classList.add('locked');
    }
...
function displayError(title, message) {
    if (document.getElementById('courseTitle')) {
        document.getElementById('courseTitle').textContent = title || 'שגיאה בטעינת הקורס';
    }
    if (document.getElementById('courseTagline')) {
        document.getElementById('courseTagline').textContent = message;
    }
    if (document.getElementById('playlistList')) {
        document.getElementById('playlistList').innerHTML =
          `<p style="color: red; text-align: center; padding: 20px;">${message}</p>`;
    }
}

// פונקציית עזר ל-הגנה על HTML
function escapeHtml(text) {
    const map = {
        '&': '&amp;',
        '<': '&lt;',
