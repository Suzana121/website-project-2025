...
// ----------------------
// טעינת רשימת קורסים דינמית (עם לוגיקת מנהל)
// ----------------------
async function loadCourses() {
    ...
    try {
        ...
        // 2. יצירת כרטיסי קורסים דינמית עם לוגיקה מותנית
        coursesContainer.innerHTML = courses.map(course => {
            let imageSrc = course.image || 'assets/default-course.png';
            
            let buttonHtml; // משתנה שיחזיק את כפתור הרכישה או הגישה

            if (isAdmin) {
                // אופציה למנהל: כפתור גישה ישירה לתוכן הקורס
                const courseUrl = `single-course.html?id=${course._id}`;
                // שימוש בתג <a> כדי לא להפעיל את handleBuyCourse
                buttonHtml = `<a href="${courseUrl}" class="buy-btn admin-btn">עבור לתוכן הקורס 👁️</a>`;
            } else {
                // אופציה לסטודנט רגיל/אורח: כפתור קנייה
                // משאיר את ה-onclick המקורי
                buttonHtml = `<button class="buy-btn" onclick="handleBuyCourse('${course._id}', ${course.price || 0})">קנו עכשיו</button>`;
            }
        ...
        // 3. עדכון כפתורי קנייה לפי סטטוס התחברות (פועל רק על כפתורי הקנייה הרגילים)
        updateBuyButtons();
